(function () {
  ...
  function installManager() {
    ...
    manager.innerHTML = `
      <div class="venue-gallery-head">
        <div>
          ...
          <h3>Add venue photos & videos</h3>
          <p>Original image files are uploaded directly. Select My Venue does not resize or recompress gallery photos.</p>
          <span class="venue-gallery-quality">ORIGINAL QUALITY UPLOAD</span>
        </div>
      ...
      <div class="venue-gallery-actions">
        <label class="venue-gallery-drop" for="venueGalleryImages">
          <span><strong>＋ Add Venue Photos</strong><small>Select multiple JPG, PNG or WebP images · up to ${MAX_IMAGES} · 20 MB each</small></span>
        </label>
        ...
        <label class="venue-gallery-drop" for="venueGalleryVideos">
          <span><strong>▶ Add Venue Videos</strong><small>MP4, WebM or MOV · up to ${MAX_VIDEOS} videos · 50 MB each</small></span>
        </label>
        <input id="venueGalleryVideos" type="file" accept="video/mp4,video/webm,video/quicktime" multiple hidden>
      </div>

      <div id="venueGalleryPreview" class="venue-gallery-preview"><div class="venue-gallery-empty">Save the venue, then its public gallery will appear here.</div></div>
      <div id="venueGalleryStatus" class="venue-gallery-status" role="status" aria-live="polite"></div>
      <div class="venue-gallery-note">For the sharpest website result, use landscape images around 1600×900 or larger. Portrait photos are accepted too and will be displayed without artificial stretching.</div>
    `;
    ...
    if (modal && !modalObserver) {
      modalObserver = new MutationObserver(() => { if (!modal.hidden) setTimeout(syncForOpenVenue, 0); });
  ...
  function mapStorageRows(rows, kind) {
    return (rows || []).filter(item => item && item.name && item.name !== ".emptyFolderPlaceholder").map(item => {
  ...
  async function loadExistingMedia() {
    ...
    if (imagesResult.error || videosResult.error) setStatus("Gallery could not be fully loaded. You can still save venue details.", "warning");
    else setStatus(currentImages.length || currentVideos.length ? "Public media gallery loaded in original quality." : "No gallery media added yet.");
  }

  function revokePendingUrls(items) {
    items.forEach(item => { if (item.previewUrl) { try { URL.revokeObjectURL(item.previewUrl); } catch (_) {} } });
  }
  ...
  async function handleImageSelection(event) {
    ...
    for (const file of files) {
      ...
      if (file.size > MAX_IMAGE_BYTES) {
        setStatus(`${file.name} is larger than 20 MB and was skipped.`, "warning");
        continue;
      }
  ...
  function handleVideoSelection(event) {
    ...
    for (const file of files) {
      ...
      if (file.size > MAX_VIDEO_BYTES) { setStatus(`${file.name} is larger than 50 MB and was skipped.`, "warning"); continue; }
      if (accepted.length >= available) break;
  ...
  async function handleRemoveClick(event) {
    ...
    if (source === "pending") {
      ...
      if (item.kind === "image") pendingImages = pendingImages.filter(x => x !== item); else pendingVideos = pendingVideos.filter(x => x !== item);
      renderGallery();
    ...
    if (item.kind === "image") currentImages = currentImages.filter(x => x !== item); else currentVideos = currentVideos.filter(x => x !== item);
    renderGallery();
