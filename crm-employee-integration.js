...
/* Manual assignment interaction bridge.
   The assignment list is rendered dynamically by the smart-matching layer.
   Keep native checkbox behaviour, but also make the whole venue row reliably
   clickable without allowing the automation layer to overwrite staff intent. */
function installManualAssignmentBridge(){
  const list=document.getElementById('venueAssignmentList');
  if(!list||list.dataset.smvManualBridge==='1')return;
  list.dataset.smvManualBridge='1';

  list.addEventListener('click',event=>{
    const item=event.target.closest?.('.venue-assignment-item');
    if(!item||!list.contains(item))return;
    const checkbox=item.querySelector('.venue-assignment-checkbox');
  ...
  list.addEventListener('change',event=>{
    const checkbox=event.target.closest?.('.venue-assignment-checkbox');
    if(!checkbox||checkbox.disabled)return;
