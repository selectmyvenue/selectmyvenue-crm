...
(function(){
 let installed=false,assignedMode=false,selectedNow=new Set(),smvAutoDeselected=new Set(),selectionLeadId='',smvSelectionTouched=false;const clean=v=>v==null?'':String(v).trim(),norm=v=>clean(v).toLowerCase();
 ...
 function smvLocationTokens(v){
   return smvText(v)
     ...
     .split(/[^a-z0-9]+/)
     .filter(x=>x&& !['delhi','ncr','new','gurgaon','gurugram','noida','greater','faridabad','ghaziabad','manesar','near','road','extension','ext'].includes(x) && (x.length>2||/^(?:sector|phase)\d+[a-z]?$/.test(x)));
 }
 ...
 async function smvGeocode(q,context='',strictSector=true){
   const db=typeof getSupabaseClient==='function'?getSupabaseClient():null;if(!db||!q)return null;
   try{const {data,error}=await db.functions.invoke('geocode-location',{body:{q,context,strict_sector:strictSector}});if(error||!data?.ok)return null;const lat=smvGeoNumber(data.latitude),lon=smvGeoNumber(data.longitude);return lat!==null&&lon!==null?{lat,lon,display_name:clean(data.display_name),provider:clean(data.provider)}:null;}catch(e){console.warn('Geocode unavailable',e);return null;}
 }
 function smvRoadOriginKey(l){const p=smvLeadPoint(l);return p?[p.lat.toFixed(5),p.lon.toFixed(5)].join(','):'';}
 function smvRoadFor(v,l){
 ...
 async function smvEnsureRoadDistances(l,venues){
   ...
   if(batchKey===smvRoadBatchKey&&rows.every(x=>x.v.__smvRoadOriginKey===originKey&&Number.isFinite(Number(x.v.__smvRoadKm))))return true;
   smvRoadBatchKey=batchKey;
   try{
     const {data,error}=await db.functions.invoke('route-distance',{body:{
       origin:{lat:origin.lat,lon:origin.lon},
       destinations:rows.map(x=>({id:String(x.v.id),lat:x.p.lat,lon:x.p.lon}))
     }});
 ...
 const smvGeoVenueAttempted=new Map();
 ...
 async function smvEnsureVenueGeo(v){
   ...
   if(storedLat!==null&&storedLon!==null)return{lat:storedLat,lon:storedLon,source:'venue'};
   const id=String(v.id||'');
   const lastAttempt=Number(smvGeoVenueAttempted.get(id)||0);
   if(lastAttempt&&Date.now()-lastAttempt<60000)return smvVenuePoint(v);
   smvGeoVenueAttempted.set(id,Date.now());

   let p=smvMapPoint(v.google_maps_url);
   if(!p){
     const mapAddress=smvMapAddress(v.google_maps_url);
     const fullAddress=mapAddress||[v.address,v.area,v.city,v.venue_name].filter(Boolean).join(', ');
     if(fullAddress)p=await smvGeocode(fullAddress,clean(v.city),false);

     // Generic fallback: if the exact Maps/full address cannot be resolved,
     // use the stored locality + city so every venue can still participate in
     // geographic ranking. This is intentionally not tied to any one sector.
     if(!p){
       const locality=[v.area,v.city].filter(Boolean).join(', ');
       if(locality)p=await smvGeocode(locality,clean(v.city),false);
     }
   }
   if(!p)return null;
   smvGeoVenueAttempted.delete(id);
   v.latitude=p.lat;v.longitude=p.lon;
   const db=typeof getSupabaseClient==='function'?getSupabaseClient():null;
   if(db&&v.id)await db.from('venues').update({latitude:p.lat,longitude:p.lon}).eq('id',v.id);
   return p;
 }
 let smvLeadGeoBackfillBusy=false;
 async function smvBackfillLeadCoordinates(){
   if(smvLeadGeoBackfillBusy||document.hidden)return;
   const rows=leads().filter(l=>{
     if(!l||smvLeadPoint(l)||!smvGeoQueryForLead(l))return false;
     const status=norm(l.status);
     return !['closed','lost','booked','not-interested'].includes(status);
   }).slice(0,4);
   if(!rows.length)return;
   smvLeadGeoBackfillBusy=true;
   try{
     for(const l of rows){
       await smvEnsureLeadGeo(l);
     }
   }finally{
     smvLeadGeoBackfillBusy=false;
   }
 }
 ...
 async function smvBackfillVenueCoordinates(){
   ...
   try{
     ...
     if(pending.length){
       ...
       try{
         if(Array.isArray(assignmentVenueRows)&&assignmentVenueRows.length){
           ...
           if(lead&&modal&&!modal.hidden){
             await smvEnsureRoadDistances(lead,assignmentVenueRows);
             smvPreparedKey='';
             setTimeout(()=>{smvSafeRenderAssignment();},0);
           }
         }
       }catch(_){}
     }
     ...
     smvVenueGeoBackfillStarted=false;
   }
 }
 async function smvRefreshAssignmentGeo(){
   ...
   try{
     ...
     // Venue coordinates may be completed by the background worker after this
     // assignment screen was opened. Re-read only missing coordinates so the
     // distance badges appear automatically without requiring another refresh.
     if(db){
       const missingIds=venues.filter(v=>!smvVenuePoint(v)).map(v=>v.id).filter(Boolean);
       if(missingIds.length){
         try{
           const {data:freshVenueGeo}=await db.from('venues')
             .select('id,latitude,longitude')
             .in('id',missingIds);
           (freshVenueGeo||[]).forEach(g=>{
             const row=venues.find(v=>String(v.id)===String(g.id));
             const lat=smvGeoNumber(g.latitude),lon=smvGeoNumber(g.longitude);
             if(row&&lat!==null&&lon!==null){row.latitude=lat;row.longitude=lon;}
           });
         }catch(_){}
       }
     }
 ...
 function smvPrepareAssignment(){
   ...
   if(hint){
     if(assigned.size>=3&&!selectedNow.size){
       hint.textContent=assigned.size+' venues are already assigned. No additional venue has been auto-selected.';
     }else{
       hint.textContent=selectedNow.size
 ...
 function resetSelectionForLead(){const l=currentAssignmentLead(),id=String(l?.id||'');if(id!==selectionLeadId){selectedNow.clear();smvAutoDeselected.clear();smvPreparedKey='';smvRoadBatchKey='';smvSelectionTouched=false;selectionLeadId=id;}}
 function smvCaptureDomSelection(){const boxes=document.querySelectorAll('#venueAssignmentList .venue-assignment-checkbox');boxes.forEach(input=>{if(input.disabled)return;const id=String(input.value);if(input.checked){smvAutoDeselected.delete(id);selectedNow.add(id);}});}
 function smvSyncSelectionInput(input){if(!input||input.disabled)return;const id=String(input.value);smvSelectionTouched=true;if(input.checked){smvAutoDeselected.delete(id);selectedNow.add(id);}else{smvAutoDeselected.add(id);selectedNow.delete(id);}renderAssignmentSummary();}
 function smvSafeRenderAssignment(){smvCaptureDomSelection();try{renderAssignmentVenues();}catch(_){} }
 function resetAssignmentUiState(){selectedNow.clear();smvAutoDeselected.clear();smvPreparedKey='';smvRoadBatchKey='';smvSelectionTouched=false;selectionLeadId='';const controls=document.getElementById('smvMatchTierControls');if(controls){controls.dataset.tier='strong';controls.querySelectorAll('[data-tier]').forEach(b=>b.classList.toggle('active',b.dataset.tier==='strong'));}const panel=document.getElementById('smvWhatsAppQueue');if(panel)panel.hidden=true;document.querySelector('.venue-assignment-note.smv-message-floating')?.classList.remove('smv-message-open');}
 ...
 function renderAssignmentSummary(){resetSelectionForLead();smvCaptureDomSelection();const wrap=ensureAssignmentSummary();if(!wrap)return;const already=assignedVenueSet();[...already].forEach(id=>selectedNow.delete(id));const assignedBox=wrap.querySelector('#smvAlreadyAssigned'),selectedBox=wrap.querySelector('#smvSelectedNow');const chips=(ids,removable)=>ids.map(id=>{const v=venueRowById(id),name=clean(v?.venue_name)||'Venue',loc=[v?.area,v?.city].filter(Boolean).join(', ');return `<span class="smv-venue-chip ${removable?'smv-selected-chip':'smv-assigned-chip'}" data-venue-id="${escapeHTML(id)}"><b>${escapeHTML(name)}</b>${loc?` <small>· ${escapeHTML(loc)}</small>`:''}${removable?'<button type="button" class="smv-chip-remove" aria-label="Remove selection">×</button>':'<button type="button" class="smv-unassign-btn" data-smv-unassign-id="'+escapeHTML(id)+'" aria-label="Unassign venue">Unassign</button>'}</span>`;}).join('');assignedBox.innerHTML=`<div class="smv-summary-title">Already Assigned <strong>${already.size}</strong></div><div class="smv-chip-row">${already.size?chips([...already],false):'<span class="smv-summary-empty">No venue assigned yet.</span>'}</div>`;selectedBox.innerHTML=`<div class="smv-summary-title">Selected Now <strong>${selectedNow.size}</strong></div><div class="smv-chip-row">${selectedNow.size?chips([...selectedNow],true):'<span class="smv-summary-empty">Select venues below — they will appear here instantly.</span>'}</div>`;}
 ...
 async function unassignVenue(venueId,button){const l=currentAssignmentLead(),v=venueRowById(venueId);if(!l)return;const name=clean(v?.venue_name)||'this venue';if(!window.confirm('Unassign '+name+' from this customer enquiry?'))return;const db=typeof getSupabaseClient==='function'?getSupabaseClient():null;if(!db){alert('CRM connection is not ready. Please refresh and try again.');return;}const old=button?.textContent;if(button){button.disabled=true;button.textContent='Removing…';}try{let a=activeAssignment(venueId);if(!a){const {data,error}=await db.from('venue_enquiry_assignments').select('*').eq('enquiry_id',l.id).eq('venue_id',venueId).neq('assignment_status','cancelled').order('assigned_at',{ascending:false}).limit(1).maybeSingle();if(error)throw error;a=data;}if(!a){alert('This venue is no longer actively assigned to this enquiry. Refreshing the assignment list.');await loadVenueAssignments();renderAssignmentSummary();try{renderAssignmentVenues();}catch(_){}return;}const {data,error}=await db.from('venue_enquiry_assignments').update({assignment_status:'cancelled'}).eq('id',a.id).select('*').single();if(error)throw error;const local=assignments().find(x=>String(x.id)===String(a.id));if(local)Object.assign(local,data||{assignment_status:'cancelled'});await loadVenueAssignments();renderAssignmentSummary();try{renderAssignmentVenues();}catch(_){}refreshCards();try{if(assignedMode)assignedFilter();}catch(_){} }catch(e){console.error('Unassign venue error',e);alert(e?.message||'Unable to unassign this venue. No assignment was changed.');}finally{if(button&&button.isConnected){button.disabled=false;button.textContent=old||'Unassign';}}}
 ...
 window.smvHandleAssignWhatsAppClick=handleAssignWhatsAppClick;
 if(document.documentElement.dataset.smvWaDelegated!=='1'){
   document.documentElement.dataset.smvWaDelegated='1';
   document.addEventListener('click',event=>{
     const b=event.target.closest?.('#smvAssignWhatsApp');
     if(!b)return;
     event.preventDefault();
     event.stopImmediatePropagation();
     handleAssignWhatsAppClick(b);
   },true);
 }
 ...
 function installWhatsAppAssignment(forceFresh=false){
   ...
   /* Click handling is delegated once at document capture phase, so it works
      identically on desktop and phone even if the modal is freshly rendered. */
   b.dataset.smvBound='delegated';
   if(forceFresh)delete b.dataset.smvBusy;
   rearmWhatsAppAssignment();
   return b;
 }
 ...
 function installAssignmentRenderer(){try{renderAssignmentVenues=function(){smvFreshAssignmentLead();smvCaptureDomSelection();setTimeout(()=>smvRefreshAssignmentGeo(),0);const list=document.getElementById('venueAssignmentList'),c=document.getElementById('assignmentVenueCount');if(!list||!assignmentCurrentLead)return;resetSelectionForLead();smvCaptureDomSelection();const already=assignedVenueSet();smvPrepareAssignment();fillMessage();let controls=document.getElementById('smvMatchTierControls');if(!controls){controls=document.createElement('div');controls.id='smvMatchTierControls';controls.className='smv-match-tier-controls';controls.innerHTML='<div class="smv-match-panel-title"><div><b>Smart Venue Matches</b><span>Scores use recorded requirements. Road distance uses OpenStreetMap / OSRM when available; confirm date availability and final quote before booking.</span></div><strong id="smvReliableMatchCount">0 reliable</strong></div><div class="smv-match-panel-tabs"><button type="button" data-tier="strong" class="active">★ Best Matches</button><button type="button" data-tier="possible">Possible 55%+</button><button type="button" data-tier="all">All Venues</button><button type="button" data-tier="assigned">Assigned</button><button type="button" data-tier="excluded">Excluded</button></div>';list.parentElement?.insertBefore(controls,list);controls.onclick=e=>{const b=e.target.closest('[data-tier]');if(!b)return;controls.querySelectorAll('button').forEach(x=>x.classList.toggle('active',x===b));controls.dataset.tier=b.dataset.tier;renderAssignmentVenues();};}const hasStrong=(assignmentVenueRows||[]).some(v=>smvMatchTier(smartMatch(v,assignmentCurrentLead))==='strong');
...
if(!venues.length){list.innerHTML='<div class="venue-assignment-empty">No approved and verified venues are available.</div>';renderAssignmentSummary();return;}list.innerHTML=venues.map(v=>{const id=String(v.id),assigned=already.has(id)||isVenueAlreadyAssigned(v.id),checked=assigned||selectedNow.has(id),match=smartMatch(v,assignmentCurrentLead),rec=recommendation(v,assignmentCurrentLead),tierLabel=(match.specificArea&&!match.areaMatch&&!match.hardFail?'Location Alternative':({requirements:'Need requirements',excluded:'Excluded',incomplete:'Venue data incomplete',strong:'Strong Match',possible:'Possible Match',low:'Low Match'})[smvMatchTier(match)]),matchChips=match.reasons.slice(0,4).map(x=>'<span class="smv-match-chip smv-match-ok">✓ '+escapeHTML(x)+'</span>').join('')+match.warnings.slice(0,2).map(x=>'<span class="smv-match-chip smv-match-warn">'+escapeHTML(x)+'</span>').join(''),cap=v.capacity_min||v.capacity_max?`${clean(v.capacity_min)||'—'}–${clean(v.capacity_max)||'—'}`:'Capacity not set',price=v.price_min_per_person||v.price_max_per_person?`₹${clean(v.price_min_per_person)||'—'}–₹${clean(v.price_max_per_person)||'—'}/person`:'Price not set',rooms=v.room_count!==null&&v.room_count!==undefined&&String(v.room_count)!==''?`${clean(v.room_count)} room${Number(v.room_count)===1?'':'s'}`:'Rooms not set',location=[v.area,v.city].filter(Boolean).join(' • ')||'Location not set',distanceLabel=match.distanceKm!==null?(match.distanceMode==='road'?'🚗 '+match.distanceKm.toFixed(1)+' km'+(match.roadMinutes!==null?' · ~'+Math.max(1,Math.round(match.roadMinutes))+' min':''):'🧭 ≈'+match.distanceKm.toFixed(1)+' km straight-line'):'';return `<label class="venue-assignment-item ${assigned?'smv-already-assigned-row':smvMatchTier(match)==='strong'?'smv-premium-match-row':''}"><input type="checkbox" class="venue-assignment-checkbox" value="${escapeHTML(v.id)}" ${checked?'checked':''} ${assigned?'disabled':''}><div class="venue-assignment-item-main"><div class="venue-assignment-item-title"><strong>${escapeHTML(v.venue_name||'Unnamed Venue')}</strong>${assigned?'<span class="smv-assigned-badge">Already Assigned</span>':match.hardFail?'<span class="smv-match-low">Excluded · must-have failed</span>':match.insufficient?'<span class="smv-match-low">Need more requirements</span>':'<span class="'+(smvMatchTier(match)==='strong'?'venue-assignment-recommended':'smv-match-score')+'">'+match.score+'% · '+tierLabel+' · data '+match.dataConfidence+'%</span>'}</div><div class="venue-assignment-item-meta"><span>📍 ${escapeHTML(location)}</span>${distanceLabel?'<span>'+escapeHTML(distanceLabel)+'</span>':''}<span>👥 ${escapeHTML(cap)}</span>${price!=='Price not set'?'<span>'+escapeHTML(price)+'</span>':''}${rooms!=='Rooms not set'&&Number(v.room_count)>0?'<span>🛏️ '+escapeHTML(rooms)+'</span>':''}<span>${escapeHTML(v.venue_type||'Venue')}</span></div><div class="smv-match-reasons">${matchChips}</div><div class="smv-readiness">${smvReadiness(v,assignmentCurrentLead).map(x=>'<span>'+escapeHTML(x)+'</span>').join('')}</div></div><span class="venue-assignment-item-status">Verified</span></label>`;}).join('');list.querySelectorAll('.venue-assignment-checkbox').forEach(input=>input.addEventListener('change',()=>smvSyncSelectionInput(input)));renderAssignmentSummary();};}catch(e){console.warn('Assignment renderer could not be installed',e);}}
 ...
 function installAssignmentInteractionGuard(){if(document.documentElement.dataset.smvAssignmentInteractionGuard==='1')return;document.documentElement.dataset.smvAssignmentInteractionGuard='1';document.addEventListener('click',e=>{const row=e.target.closest?.('#venueAssignmentList .venue-assignment-item');if(row){const input=row.querySelector('.venue-assignment-checkbox');if(input&&!input.disabled)e.stopImmediatePropagation();}},true);document.addEventListener('change',e=>{const input=e.target.closest?.('#venueAssignmentList .venue-assignment-checkbox');if(!input||input.disabled)return;e.stopImmediatePropagation();smvSyncSelectionInput(input);},true);}
 function installUnassignClicks(){if(document.documentElement.dataset.smvUnassignDelegated==='1')return;document.documentElement.dataset.smvUnassignDelegated='1';document.addEventListener('click',e=>{const b=e.target.closest?.('.smv-unassign-btn,[data-smv-unassign-id]');if(!b)return;const id=String(b.dataset.smvUnassignId||b.closest?.('.smv-venue-chip')?.dataset.venueId||'');if(!id)return;e.preventDefault();e.stopImmediatePropagation();unassignVenue(id,b);},true);}
 ...
 window.addEventListener('focus',()=>{installWhatsAppAssignment(false);rearmWhatsAppAssignment();smvBackfillLeadCoordinates();});
 window.addEventListener('pageshow',()=>{installWhatsAppAssignment(false);rearmWhatsAppAssignment();smvBackfillLeadCoordinates();});
