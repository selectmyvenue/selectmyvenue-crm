...
(function(){
 ...
 function smvCaptureDomSelection(){const boxes=document.querySelectorAll('#venueAssignmentList .venue-assignment-checkbox');boxes.forEach(input=>{if(input.disabled)return;const id=String(input.value);if(input.checked){smvAutoDeselected.delete(id);selectedNow.add(id);}else if(smvSelectionTouched){smvAutoDeselected.add(id);selectedNow.delete(id);}});}
...
if(!venues.length){list.innerHTML='<div class="venue-assignment-empty">No approved and verified venues are available.</div>';renderAssignmentSummary();return;}list.innerHTML=venues.map(v=>{const id=String(v.id),assigned=already.has(id)||isVenueAlreadyAssigned(v.id),checked=assigned||selectedNow.has(id),match=smartMatch(v,assignmentCurrentLead),rec=recommendation(v,assignmentCurrentLead),tierLabel=(match.specificArea&&!match.areaMatch&&!match.hardFail?'Location Alternative':({requirements:'Need requirements',excluded:'Excluded',incomplete:'Venue data incomplete',strong:'Strong Match',possible:'Possible Match',low:'Low Match'})[smvMatchTier(match)]),matchChips=match.reasons.slice(0,4).map(x=>'<span class="smv-match-chip smv-match-ok">✓ '+escapeHTML(x)+'</span>').join('')+match.warnings.slice(0,2).map(x=>'<span class="smv-match-chip smv-match-warn">'+escapeHTML(x)+'</span>').join(''),cap=v.capacity_min||v.capacity_max?`${clean(v.capacity_min)||'—'}–${clean(v.capacity_max)||'—'}`:'Capacity not set',price=v.price_min_per_person||v.price_max_per_person?`₹${clean(v.price_min_per_person)||'—'}–₹${clean(v.price_max_per_person)||'—'}/person`:'Price not set',rooms=v.room_count!==null&&v.room_count!==undefined&&String(v.room_count)!==''?`${clean(v.room_count)} room${Number(v.room_count)===1?'':'s'}`:'Rooms not set',location=[v.area,v.city].filter(Boolean).join(' • ')||'Location not set',distanceLabel=match.distanceKm!==null?(match.distanceMode==='road'?'🚗 '+match.distanceKm.toFixed(1)+' km'+(match.roadMinutes!==null?' · ~'+Math.max(1,Math.round(match.roadMinutes))+' min':''):'🧭 ≈'+match.distanceKm.toFixed(1)+' km straight-line'):'';return `<label class="venue-assignment-item ${assigned?'smv-already-assigned-row':smvMatchTier(match)==='strong'?'smv-premium-match-row':''}"><input type="checkbox" class="venue-assignment-checkbox" value="${escapeHTML(v.id)}" ${checked?'checked':''} ${assigned?'disabled':''}><div class="venue-assignment-item-main"><div class="venue-assignment-item-title"><strong>${escapeHTML(v.venue_name||'Unnamed Venue')}</strong>${assigned?'<span class="smv-assigned-badge">Already Assigned</span>':match.hardFail?'<span class="smv-match-low">Excluded · must-have failed</span>':match.insufficient?'<span class="smv-match-low">Need more requirements</span>':'<span class="'+(smvMatchTier(match)==='strong'?'venue-assignment-recommended':'smv-match-score')+'">'+match.score+'% · '+tierLabel+' · data '+match.dataConfidence+'%</span>'}</div><div class="venue-assignment-item-meta"><span>📍 ${escapeHTML(location)}</span>${distanceLabel?'<span>'+escapeHTML(distanceLabel)+'</span>':''}<span>👥 ${escapeHTML(cap)}</span>${price!=='Price not set'?'<span>'+escapeHTML(price)+'</span>':''}${rooms!=='Rooms not set'&&Number(v.room_count)>0?'<span>🛏️ '+escapeHTML(rooms)+'</span>':''}<span>${escapeHTML(v.venue_type||'Venue')}</span></div><div class="smv-match-reasons">${matchChips}</div><div class="smv-readiness">${smvReadiness(v,assignmentCurrentLead).map(x=>'<span>'+escapeHTML(x)+'</span>').join('')}</div></div><span class="venue-assignment-item-status">Verified</span></label>`;}).join('');list.querySelectorAll('.venue-assignment-checkbox').forEach(input=>input.addEventListener('change',()=>smvSyncSelectionInput(input)));renderAssignmentSummary();};}catch(e){console.warn('Assignment renderer could not be installed',e);}}
 ...
 function installAssignmentInteractionGuard(){/* Intentionally no event interception. Native checkbox/label behavior is required for manual assignment. */}
 function installUnassignClicks(){if(document.documentElement.dataset.smvUnassignDelegated==='1')return;document.documentElement.dataset.smvUnassignDelegated='1';document.addEventListener('click',e=>{const b=e.target.closest?.('.smv-unassign-btn,[data-smv-unassign-id]');if(!b)return;const id=String(b.dataset.smvUnassignId||b.closest?.('.smv-venue-chip')?.dataset.venueId||'');if(!id)return;e.preventDefault();unassignVenue(id,b);},true);}
 ...
 window.smvGetAssignmentSelection=()=>{smvCaptureDomSelection();const domChecked=[...document.querySelectorAll('#venueAssignmentList .venue-assignment-checkbox:checked:not(:disabled)')].map(x=>String(x.value));return [...new Set([...domChecked,...selectedNow])].filter(id=>!smvAutoDeselected.has(String(id)));};
