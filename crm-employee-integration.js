...
(function(){
 ...
 function smvGeoQueryForLead(l){
   ...
   if(area)return [area,'Delhi NCR','India'].join(', ');
   if(!city||smvIsBroadLocation(city))return'';
   return [city,'Delhi NCR','India'].join(', ');
 }
 ...
 async function smvRefreshAssignmentGeo(){
   ...
   try{
     const hint=document.getElementById('smvPreparationStatus');if(hint&&!smvLeadPoint(lead)&&smvGeoQueryForLead(lead))hint.textContent='Refining nearest venues from Venue / Area…';
     ...
     const pending=venues.filter(v=>!smvVenuePoint(v)&&(!region||smvRegion([v.area,v.city,v.address].filter(Boolean).join(' '))===region)).slice(0,6);
