...
(function(){
 ...
 async function smvGeocode(q,context=''){
   const db=typeof getSupabaseClient==='function'?getSupabaseClient():null;if(!db||!q)return null;
   try{const {data,error}=await db.functions.invoke('geocode-location',{body:{q,context}});if(error||!data?.ok)return null;const lat=smvGeoNumber(data.latitude),lon=smvGeoNumber(data.longitude);return lat!==null&&lon!==null?{lat,lon,display_name:clean(data.display_name),provider:clean(data.provider)}:null;}catch(e){console.warn('Geocode unavailable',e);return null;}
 }
...
list.querySelectorAll('.venue-assignment-checkbox').forEach(checkbox=>{
  checkbox.addEventListener('change',()=>{
    ...
    smvSelectionTouched=true;
    if(checkbox.checked)selectedNow.add(id);
    else selectedNow.delete(id);
    renderAssignmentSummary();
  });
});
renderAssignmentSummary();};}catch(e){console.warn('Assignment renderer could not be installed',e);}}
 ...
 function installUnassignClicks(){
   if(document.documentElement.dataset.smvUnassignBridge==='1')return;
   document.documentElement.dataset.smvUnassignBridge='1';
   document.addEventListener('click',e=>{
     const b=e.target.closest?.('.smv-unassign-btn');
     if(!b)return;
     const chip=b.closest('.smv-venue-chip');
     const id=chip?.dataset.venueId;
     if(!id||b.dataset.smvBusy==='1')return;
     e.preventDefault();
     e.stopImmediatePropagation();
     b.dataset.smvBusy='1';
     Promise.resolve(unassignVenue(id,b)).finally(()=>{delete b.dataset.smvBusy;});
   },true);
}
